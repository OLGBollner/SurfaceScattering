...
R_true /= (R_true.sum() * deltaAlphaP * deltaBetaP)

# Simulated M
M_true = deltaAlphaP * I @ R_true
M_true /= M_true.sum()

# Simulate MP using R_true
MP_simulated = deltaBetaP * M_true @ f.T
MP_simulated /= MP_simulated.sum()

print(f"M: {M_true.shape}")
...
# Plot R
plt.figure(figsize=(12, 5))
plt.plot(np.rad2deg(alphaP), R[100, :], linestyle='-', label=f'Reflectivity R, Beta {np.rad2deg(betaVal):.1f} (deg)')
plt.plot(np.rad2deg(alphaP), R_true[100, :], linestyle='-.', label=f'Generated R_true, Beta {np.rad2deg(betaVal):.1f} (deg)')
